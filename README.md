...
- step3 \
    - Modeling(Auto encoder). 모델에 대한 자세한 정보는 [포스터]()에서 확인 가능.
...
그러나 model1이 재구성한 이미지를 시각화해본 결과, PCA와 비슷한 원리로 linear reduction을 통해, 섬유의 특징을 섬세하게 추출해내지 못했음을 확인할 수 있다.

![model4-test](./model4.png)
따라서 validation에서 좋은 성능을 보였고, validate 데이터의 예측 및 재구성 결과도 좋은 model4를 최종 모델로 선정한다.

- step4 \
테스트 셋에 대한 f1-scroe = 81%. \
